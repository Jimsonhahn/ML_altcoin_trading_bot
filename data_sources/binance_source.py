...
class BinanceDataSource(DataSourceBase):
    ...
    # Update für data_sources/data_sources/binance_source.py
    ...
    def get_historical_data(self, symbol: str, timeframe: str = "1h",
        ...
        if not use_demo_data:
            try:
                ...
                # Überprüfen, ob das DataFrame einen Zeitindex hat
                if not df.empty and not isinstance(df.index, pd.DatetimeIndex):
                    try:
                        # Versuchen, den Index zu einem Zeitindex zu konvertieren
                        df.index = pd.to_datetime(df.index)
                    except Exception as e:
                        self.logger.warning(f"Konnte DataFrame-Index nicht zu einem Zeitindex konvertieren: {e}")

                        # Wenn es eine 'timestamp'-Spalte gibt, diese als Index verwenden
                        if 'timestamp' in df.columns:
                            self.logger.info("Versuche timestamp-Spalte als Index zu verwenden")
                            try:
                                ...
                                # Fallback für Zeichenketten-Datum
                                if df['date'].isna().any():
                                ...
                                self.logger.warning(f"Auch die Konvertierung der timestamp-Spalte fehlgeschlagen: {e2}")
    ...
    def get_historical_prices(self, symbol: str, timeframe: str = "1h",
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Konnte timestamp nicht als Millisekunden konvertieren: {e}, versuche alternative Methode")
                try:
                    # Direkte Konvertierung ohne unit
                    df['timestamp'] = pd.to_datetime(df['timestamp'])
                    df.set_index('timestamp', inplace=True)
                except Exception as e2:
                    self.logger.error(f"Konnte timestamp nicht konvertieren: {e2}")
                    # Erzeuge einen Dummy-Datetime-Index für den Notfall
                    df['date'] = pd.date_range(start=datetime.now(), periods=len(df), freq='H')
                    df.set_index('date', inplace=True)
