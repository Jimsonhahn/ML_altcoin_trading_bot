...
# Ensure autopilot is registered
try:
    if 'autopilot' not in STRATEGIES:
        from .autopilot import UltimateAutoPilotStrategy
        STRATEGIES['autopilot'] = UltimateAutoPilotStrategy
        print(f"AutoPilot registered. Total strategies: {len(STRATEGIES)}")
except:
    pass

# AutoPilot Registration (added by instant_fix.py)
try:
    from .autopilot import UltimateAutoPilotStrategy
    STRATEGIES['autopilot'] = UltimateAutoPilotStrategy
except:
